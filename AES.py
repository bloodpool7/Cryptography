...
def expand_key(key, rounds):
    ...
    for i in range(4 * rounds):
        if i < n:
            ...
            rcon_applied = [xor_hex_strings(rcon[i//n - 1][j], x) if len(xor_hex_strings(rcon[i//n - 1][j], x)) == 2 else "0" + xor_hex_strings(rcon[i//n - 1][j], x) for j, x in enumerate(subbed_word)]
            final_word = [xor_hex_strings(words[i-n][j], x) if len(xor_hex_strings(words[i-n][j], x)) == 2 else "0" + xor_hex_strings(words[i-n][j], x) for j, x in enumerate(rcon_applied)]
            words.append(final_word)
        elif n > 6 and i % n == 4:
            subbed_word = [lookup(x) for x in words[i-1]]
            words.append([xor_hex_strings(words[i-n][j], x) if len(xor_hex_strings(words[i-n][j], x)) == 2 else "0" + xor_hex_strings(words[i-n][j], x) for j, x in enumerate(subbed_word)])
        else:
            words.append([xor_hex_strings(words[i-1][j], x) if len(xor_hex_strings(words[i-1][j], x)) == 2 else "0" + xor_hex_strings(words[i-1][j], x) for j, x in enumerate(words[i-n])])
...
def pad(input, num):
    ...
    for i in range(num):
        if num - 10 < 0:
            final += bytes.fromhex("0" + str(num)).hex()
        else:
            final += bytes.fromhex(str(num)).hex()
    return final

def unpad(input):
    final_pt = bytes.fromhex(input)
    valid = True
    if final_pt[-1] < 10 or (final_pt[-1] >= 0x10 and final_pt[-1] < 0x17):
        for i in range(1, int(hex(final_pt[-1])[2:]) + 1):
            if final_pt[-i] != final_pt[-1]:
                valid = False 
    else :
        valid = False
...
def encrypt_ecb(block, key):
    rounds = 10 if len(key) == 32 else (12 if len(key) == 48 else 14)
    keys = expand_key(key, rounds+1)
    ...
    for block1 in blocks:
        ...
        final_state = matrix_to_bytes(xor_byte_matrices(state, key_matricies[rounds]))
        encrypted_blocks.append(final_state)

    ciphertext = "".join(encrypted_blocks)
    return ciphertext

def decrypt_ecb(block, key):
    rounds = 10 if len(key) == 32 else (12 if len(key) == 48 else 14)
    keys = expand_key(key, rounds + 1)
    key_matricies = [bytes_to_matrix(x) for x in keys]
    blocks = break_into_16_bytes(block)
...
def encrypt_cbc(msg, key, iv):
    ...
    blocks[-1] = pad(blocks[-1], (32 - len(blocks[-1])) // 2)

    prev_state = ""
    ciphertext = [iv]
    for i in range(len(blocks)):
        if i == 0:
            prev_state = encrypt_ecb(xor_hex_strings_samelen(blocks[i], iv), key)
            ciphertext.append(prev_state)
        else:
            ciphertext.append(encrypt_ecb(xor_hex_strings_samelen(blocks[i], prev_state), key))
            prev_state = encrypt_ecb(xor_hex_strings_samelen(blocks[i], prev_state), key)
...
def decrypt_cbc(cipher, key):
    ...
    for i in range(1, len(blocks)):
        plaintext.append(xor_hex_strings_samelen(decrypt_ecb(blocks[i], key), blocks[i-1]))
    
    plaintext[-1] = unpad(plaintext[-1])
...
def encrypt_ctr(plaintext, key, iv):
    ...
    ciphertext = [iv]
    for i in range(len(blocks)):
        encrypted_iv = encrypt_ecb(hex(int(iv, 16) + i)[2:], key)
...
def decrypt_ctr(ciphertext, key):
    ...
    plaintext = []
    for i in range(1, len(blocks)):
        decrypted_iv = encrypt_ecb(hex(int(blocks[0], 16) + (i-1))[2:], key)
...
if __name__ == "__main__":
    ...
    message = string_to_hex("Congrats for reading this. I think I'm hilarious and amazing")
    iv = string_to_hex("!!Rishabh Goel!!")

    ct = encrypt_ecb(pad(message, 4), key)
    print(ct)
